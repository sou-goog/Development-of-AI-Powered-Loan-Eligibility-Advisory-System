          ...
          <ul className="space-y-2">
            {sidebarLinks.map((link) => (
              <li key={link.to}>
                <Link
                  to={link.to}
                  className={`block px-4 py-2 rounded-lg transition-colors font-medium ${location.pathname === link.to ? 'bg-slate-700 text-blue-400' : 'hover:bg-slate-800 hover:text-blue-300'}`}
                >
                  {link.label}
                </Link>
              </li>
